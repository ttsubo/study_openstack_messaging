...
def spawn_with(ctxt, pool):
    ...
    """
    LOG.debug("### spawn_with()")

    def complete(thread, exit):
        LOG.debug("### spawn_with().complete()")
        exc = True
        try:
            try:
                thread.wait()
...
class EventletExecutor(base.ExecutorBase):
    ...
    def __init__(self, conf, listener, dispatcher):
        LOG.debug("### EventletExecutor.__init__()")
    ...
    def start(self):
        LOG.debug("### EventletExecutor.start()")
    ...
    def stop(self):
        LOG.debug("### EventletExecutor.stop()")
        if self._thread is None:
            return
        self._thread.kill()

    def wait(self):
        LOG.debug("### EventletExecutor.wait()")
        if self._thread is None:
            return
