...
class AMQPIncomingMessage(base.IncomingMessage):
    ...
    def __init__(self, listener, ctxt, message, unique_id, msg_id, reply_q):
        LOG.debug("### AMQPIncomingMessage.__init__()")
    ...
    def _send_reply(self, conn, reply=None, failure=None,
        ...
        LOG.debug("### AMQPIncomingMessage._send_reply()")
        if failure:
    ...
    def reply(self, reply=None, failure=None, log_failure=True):
        LOG.debug("### AMQPIncomingMessage.reply()")
        if not self.msg_id:
    ...
    def acknowledge(self):
        LOG.debug("### AMQPIncomingMessage.acknowledge()")
    ...
    def requeue(self):
        ...
        # the end.
        LOG.debug("### AMQPIncomingMessage.requeue()")
...
class AMQPListener(base.Listener):
    ...
    def __init__(self, driver, conn):
        LOG.debug("### AMQPListener.__init__()")
    ...
    def __call__(self, message):
        ...
        LOG.debug("### AMQPListener.__call__()")
...
class ReplyWaiters(object):
    ...
    def __init__(self):
        LOG.debug("### ReplyWaiters.__init__()")
        self._queues = {}
        self._wrn_threshold = 10

    def get(self, msg_id, timeout):
        LOG.debug("### ReplyWaiters.get()")
        try:
...
class ReplyWaiter(object):
    ...
    def __init__(self, conf, reply_q, conn, allowed_remote_exmods):
        LOG.debug("### ReplyWaiter.__init__()")
        self.conf = conf
        self.conn = conn
        self.reply_q = reply_q
    ...
    def _process_reply(self, data):
        LOG.debug("### ReplyWaiter._process_reply()")
        result = None
        ending = False
    ...
    def _poll_connection(self, msg_id, timer):
        LOG.debug("### ReplyWaiter._poll_connection()")
        while True:
            while self.incoming:
    ...
    def _poll_queue(self, msg_id, timer):
        LOG.debug("### ReplyWaiter._poll_queue()")
        timeout = timer.check_return(self._raise_timeout_exception, msg_id)
    ...
    def _check_queue(self, msg_id):
        LOG.debug("### ReplyWaiter._check_queue()")
        while True:
...
class AMQPDriverBase(base.BaseDriver):
    ...
    def __init__(self, conf, url, connection_pool,
        ...
        LOG.debug("### AMQPDriverBase.__init__()")
    ...
    def _get_exchange(self, target):
        LOG.debug("### AMQPDriverBase._get_exchange()")
        return target.exchange or self._default_exchange

    def _get_connection(self, pooled=True):
        LOG.debug("### AMQPDriverBase._get_connection()")
    ...
    def _get_reply_q(self):
        with self._reply_q_lock:
            LOG.debug("### AMQPDriverBase._get_reply_q()")
    ...
    def _send(self, target, ctxt, message,
        ...
        LOG.debug("### AMQPDriverBase._send()")
    ...
    def send(self, target, ctxt, message, wait_for_reply=None, timeout=None,
             retry=None):
        LOG.debug("### AMQPDriverBase.send()")
        return self._send(target, ctxt, message, wait_for_reply, timeout,
                          retry=retry)

    def send_notification(self, target, ctxt, message, version, retry=None):
        LOG.debug("### AMQPDriverBase.send_notification()")
        return self._send(target, ctxt, message,
                          envelope=(version == 2.0), notify=True, retry=retry)

    def listen(self, target):
        LOG.debug("### AMQPDriverBase.listen()")
        conn = self._get_connection(pooled=False)
    ...
    def listen_for_notifications(self, targets_and_priorities):
        LOG.debug("### AMQPDriverBase.listen_for_notifications()")
        conn = self._get_connection(pooled=False)
    ...
    def cleanup(self):
        LOG.debug("### AMQPDriverBase.cleanup()")
